...
- **Add: opt-in 1M context per model** — list bare model ids under `provider.enableLongContextModels` in `~/.pi/agent/claude-bridge.json` or `.pi/claude-bridge.json` to enable 1M for those models; the default is the 200K window for every model (which on Max/Team/Enterprise still yields Opus 1M via Claude Code's bare-id auto-upgrade, default since v2.1.75). The bridge appends `[1m]` to the CLI model id for listed long-context models, and registers the matching `contextWindow` with pi via `applyLongContext(MODELS, set)` at the `registerProvider` site, so pi's status bar and auto-compaction threshold reflect the runtime window — preventing the issue #24/#17 "Prompt is too long" / credit-gate class where pi reports 1M headroom while CC runs at 200K. Display names stay bare. Only 1M-capable models (Opus 4.6/4.7/4.8, Sonnet 4.6) honor an opt-in; Haiku 4.5 (200K) and unlisted ids stay at 200K. Entitlement is model- and plan-specific: Sonnet 4.6's 1M is metered via usage credits on *every* plan (including Max); Opus 1M is auto-upgraded on Max/Team/Enterprise so don't opt in there (an explicit `opus[1m]` is redundant and reportedly trips the credits gate on Max 5x, claude-code#39841); on Pro, Opus 1M requires usage credits. Caveat: an unlisted Opus on Max/Team/Enterprise registers 200K but runs at 1M via auto-upgrade, so pi compacts earlier than the true headroom allows (safe, wasteful) — list it to get accurate 1M budgeting, accepting the #39841 risk.
