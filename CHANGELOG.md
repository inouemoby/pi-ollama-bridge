...
- **Fix: route subagent calls as reentrant queries** — user-only provider calls that arrive while a parent Claude Code query is active now start a nested bridge query instead of being mistaken for empty tool-result delivery, covering foreground and background subagents regardless of inherited context size or pending-handler timing.
- **Tests: add rpiv codebase-locator subagent regression** — the integration suite now loads `@tintinweb/pi-subagents` 0.6.3 with rpiv-pi v0.6.0's `codebase-locator` agent and verifies foreground and background `Agent` calls complete through claude-bridge without the stuck MCP-handler signature.
- **Tests: harden manual compact integration seeds** — baseline and session compact tests now force a low `keepRecentTokens` setting in isolated agent dirs so concise provider responses do not make `/compact` report "Nothing to compact".
