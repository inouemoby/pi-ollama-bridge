...
- **Fix: route foreground subagent calls as reentrant queries** — user-only provider calls that arrive while a parent Claude Code query is waiting on an `Agent` MCP handler now start a nested bridge query instead of being mistaken for empty tool-result delivery and stranding the handler.
- **Tests: add rpiv codebase-locator subagent regression** — the integration suite now loads `@tintinweb/pi-subagents` 0.6.3 with rpiv-pi v0.6.0's `codebase-locator` agent and verifies the foreground `Agent` call completes through claude-bridge without the stuck MCP-handler signature.
